...
export async function checkForChanges(
  ...
  try {
    const txResp = await client.getTransactions({ pageSize: 50 });
    transactions = txResp.items ?? [];
  } catch (e: any) {
  ...
  try {
    ...
    allSubs = Object.fromEntries(
      (["active", "trialing", "past_due", "paused", "canceled", "expired", "scheduled_cancel"] as SubscriptionStatus[])
        .map(s => [s, []])
    ) as Record<SubscriptionStatus, CreemSubscription[]>;
  }

  try {
    const custResp = await client.getCustomers();
  ...
  const newCustomers = prev.customerCount > 0 && customers.length > prev.customerCount
    ? customers.slice(0, customers.length - prev.customerCount)
