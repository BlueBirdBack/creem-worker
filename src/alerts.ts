...
export function fmtSubscriptionChange(change: SubscriptionChange): string {
  ...
  const header = change.newStatus === "past_due"
    ? "🚨 Payment Failed"
    : change.newStatus === "canceled" || change.newStatus === "scheduled_cancel"
  ...
  } else if (change.newStatus === "scheduled_cancel") {
...
export function fmtChangeSet(changes: ChangeSet): string | null {
  ...
  const cancelCount = changes.subscriptionChanges.filter(
    sc => sc.newStatus === "canceled" || sc.newStatus === "expired" || sc.newStatus === "scheduled_cancel"
  ).length;
  if (cancelCount >= 3) {
    parts.push("");
...
export function fmtDailyDigest(data: DailyDigestData): string {
  ...
  const statuses: SubscriptionStatus[] = ["active", "trialing", "past_due", "paused", "scheduled_cancel", "canceled", "expired"];
...
export function fmtStoreSnapshot(data: {
  ...
  const statuses: SubscriptionStatus[] = ["active", "trialing", "past_due", "paused", "scheduled_cancel"];
  for (const s of statuses) {
    const count = data.subscriptions[s] ?? 0;
